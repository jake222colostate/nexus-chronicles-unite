...
interface BottomActionBarProps {
  ...
  onRealmChange: (realm: 'fantasy' | 'scifi' | 'nexus') => void;
  onTap: () => void;
...
export const BottomActionBar: React.FC<BottomActionBarProps> = ({
  ...
  const handleRealmSwitch = (realm: 'fantasy' | 'scifi' | 'nexus') => {
    ...
    <div className="absolute bottom-0 left-0 right-0 z-30">
      ...
      <div className="px-4 pb-2">
        <div className="text-center mb-1">
          <span className="text-white/90 text-xs font-medium">
            Journey: {Math.floor(playerDistance)}m | Realm: {currentRealm === 'fantasy' ? 'Fantasy' : currentRealm === 'scifi' ? 'Sci-Fi' : 'Nexus'}
          </span>
        </div>
      ...
      <div className="bg-black/95 backdrop-blur-xl border-t border-white/20 py-6">
        <div className="flex items-center justify-center px-6">
            ...
            <Button 
              ...
              className={`h-16 w-48 rounded-full transition-all duration-200 hover:scale-110 active:scale-92 font-bold text-3xl backdrop-blur-xl border-3 relative overflow-hidden shadow-2xl ${
                currentRealm === 'fantasy'
                  ...
                  : 'bg-gradient-to-br from-indigo-600/95 to-purple-700/95 hover:from-indigo-500/95 hover:to-purple-600/95 border-indigo-400/80 text-indigo-100'
              }`}
              style={{
                boxShadow: `0 0 60px ${currentRealm === 'fantasy' ? 'rgba(168, 85, 247, 0.9)' : currentRealm === 'scifi' ? 'rgba(34, 211, 238, 0.9)' : 'rgba(99, 102, 241, 0.9)'}, 0 10px 40px rgba(0,0,0,0.6)`,
              }}
            >
          ...
          {/* Nexus Button - positioned below */}
          <div className="flex justify-center mt-4">
            <Button
              onClick={() => handleRealmSwitch('nexus')}
              disabled={isTransitioning}
              className={`h-12 px-6 rounded-xl transition-all duration-300 hover:scale-105 active:scale-95 relative overflow-hidden font-medium border-2 ${
                currentRealm === 'nexus'
                  ? 'bg-indigo-600/95 hover:bg-indigo-700/95 border-indigo-400/80 text-indigo-100 scale-105'
                  : 'bg-black/70 border-indigo-400/60 text-indigo-300 hover:bg-indigo-900/50 hover:border-indigo-400/80 hover:text-indigo-200'
              } ${isTransitioning ? 'opacity-50' : 'opacity-100'}`}
              style={{
                boxShadow: currentRealm === 'nexus' 
                  ? '0 4px 20px rgba(99, 102, 241, 0.6), 0 2px 10px rgba(0,0,0,0.4)' 
                  : '0 2px 10px rgba(0,0,0,0.3)'
              }}
            >
              <span className="relative z-10 text-sm font-bold">🌌 Nexus</span>
              {currentRealm === 'nexus' && (
                <div className="absolute inset-0 bg-indigo-400/25 animate-pulse rounded-xl" />
              )}
            </Button>
          </div>
        </div>
