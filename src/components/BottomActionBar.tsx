...
export const BottomActionBar: React.FC<BottomActionBarProps> = ({
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-30 iphone-safe-bottom">
      {/* Moved deeper into footer zone with enhanced 3-button layout */}
      <div className="flex items-center justify-center px-4 pb-6 pt-4">
        <div className="flex items-center justify-between w-full max-w-lg gap-4">
          {/* Fantasy Realm Button - enhanced with better animations */}
          <Button
            onClick={() => handleRealmSwitch('fantasy')}
            disabled={isTransitioning}
            className={`h-12 px-6 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-lg active:scale-95 relative overflow-hidden font-medium text-sm border-2 flex-shrink-0 ${
          ...
          {/* Enhanced Oval Tap Button - Perfect Center with horizontal oval shape */}
          <div className="flex-shrink-0">
            <Button 
              id="tap-button"
              onClick={handleTap}
              className={`h-16 w-24 rounded-full transition-all duration-200 hover:scale-110 hover:shadow-2xl active:scale-95 font-medium text-3xl backdrop-blur-xl border-3 relative overflow-hidden ${
                currentRealm === 'fantasy'
          ...
          {/* Sci-Fi Realm Button - enhanced with better animations */}
          <Button
