...
export const ChunkSystem: React.FC<ChunkSystemProps> = React.memo(({
  ...
  const activeChunks = useMemo(() => {
    ...
    const maxRenderDistance = Math.min(renderDistance, 100); // Reduced from 250 to 100
    const chunkRadius = Math.ceil(maxRenderDistance / chunkSize); // Removed buffer
    const farAheadChunks = Math.ceil(maxRenderDistance / chunkSize); // Removed buffer
    
    // PERFORMANCE FIX: Much lower chunk limit
    let chunkCount = 0;
    const maxChunks = 50; // Reduced from 500 to 50
