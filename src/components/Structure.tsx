...
export const Structure: React.FC<StructureProps> = ({
  ...
  return (
    ...
    >
      {/* Structure Visual */}
      <div 
        className={`${sizeClasses[position.size]} relative cursor-pointer group transition-all duration-300 hover:scale-110`}
        onClick={() => setShowDetails(!showDetails)}
      >
        {/* Base Structure */}
        <div className={`w-full h-full rounded-lg border-2 flex items-center justify-center text-4xl relative overflow-hidden ${
          realm === 'fantasy'
          ...
          {count > 0 && (
            <div className="absolute inset-0">
              {realm === 'fantasy' ? (
                <div className="absolute top-2 left-2 w-2 h-2 bg-purple-400 rounded-full animate-bounce opacity-70" />
              ) : (
                <div className="absolute top-2 right-2 w-1 h-4 bg-cyan-400 animate-pulse opacity-70" />
              )}
            </div>
          )}
      ...
      {showDetails && (
        <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 z-10">
          <Card className={`p-4 w-64 backdrop-blur-sm ${
            realm === 'fantasy'
            ...
            <div className="text-white space-y-2">
              ...
              <p className="text-sm opacity-80">{building.description}</p>
