...
export const ScifiScrollUpgradeSystem: React.FC<ScifiScrollUpgradeSystemProps> = ({
  ...
  const upgradePositions = useMemo(() => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      // Position upgrades on the same plane as other visible upgrades (around z=0 to z=-5)
      const minZ = 0; // Start at z=0 like other upgrades
      const z = minZ - (layer * 1) + Math.random() * 1; // Slight depth variation, staying close to z=0 plane
