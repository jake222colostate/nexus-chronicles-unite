...
interface AutoClickerUpgradeSystemProps {
  currentMana: number;
  onUpgrade: (cost: number) => void;
}

export const AutoClickerUpgradeSystem: React.FC<AutoClickerUpgradeSystemProps> = ({
  currentMana,
  onUpgrade
}) => {
      ...
      <div className="flex flex-col items-center gap-1">
        <Button
          ...
          className={`h-10 px-4 rounded-xl backdrop-blur-xl border transition-all duration-300 font-bold shadow-lg ${
            canAfford
              ? 'bg-gradient-to-r from-purple-500/95 to-indigo-500/95 hover:from-purple-600/95 hover:to-indigo-600/95 border-purple-400/70 shadow-purple-500/30 text-white'
              : 'bg-gray-600/50 border-gray-500/50 text-gray-400 cursor-not-allowed'
          }`}
        >
          🤖 AutoClicker Lvl {level} - {upgradeCost} Mana
        </Button>
